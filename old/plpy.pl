...
sub translate_pl_line {
    ...
    if ($contents && $contents =~ m{\w+}) {
        ...
        if ($contents =~ m{^#!} && $. == 1) {
            ...
            $contents .= "\n";
        } elsif ($contents =~ m{print\s*(.*)}) {
            # handle print statements
            # $1 - all content
            $contents = handle_print($1);
        } elsif ($contents =~ m{(if|elsif|while)\s*\((.*)\)}) {
        ...
        } elsif ($contents =~ m{^(last|next)$}) {
            # handle last/next
            # $1 - all content
            $contents = handle_last_next($1);
        } elsif ($contents =~ m{chomp\s+(.*)$}) {
            # handle chomp
            # $1 - var to chomp
            $contents = handle_chomp($1);
        } elsif ($contents =~ m{^open (\w+), "<(\w+)"}) {
            $contents = handle_open($1, $2);
        }
